...
try:
    ...
    # Error Plot (imshow requires 2D data, error is (Nx, Ny))
    ax3 = fig.add_subplot(133)
    # Transpose error for imshow to match x,y axes orientation
    error_plot = ax3.imshow(error.T, extent=[0, x[Nx-1], -b, b], origin='lower', aspect='auto', cmap='hot')
    ax3.set_title(f'Absolute Error (Max: {max_error:.2e})')
    ax3.set_xlabel('x')
    ax3.set_ylabel('y')
    fig.colorbar(error_plot, shrink=0.5, aspect=5)

    plt.tight_layout()
    plot_filename = "pde_solution_comparison.png"
